...
int write_h(info_t *info)
{
	...
	{
		puts_to_file(node->str, fd);
		_put_to_file('\n', fd);
	}
	_put_to_file(BUF_FLUSH, fd);
	close(fd);
	return (1);
}
...
int read_h(info_t *info)
{
	...
	struct stat st;
	char *buf = NULL, *filename = get_h(info);
	...
	while (info->histcount-- >= HIST_MAX)
		delete_node_with_index(&(info->history), 0);
	renumber_h(info);
