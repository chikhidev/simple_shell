...
ssize_t buffer_input(info_t *info, char **buf, size_t *len)
{
	...
	{
		/*free_time_salah((void **)info->cmd_buf);*/
		free(*buf);
		*buf = NULL;
...
/**
 * input_taker - gets a line minus the newline
 * @info: parameter struct
 *
 * Return: bytes read
 */
ssize_t input_taker(info_t *info)
{
	...
	if (len)	/* we have commands left in the chain buffer */
	{
		...
		check_chain_size(info, buf, &j, i, len);
		while (j < len) /* iterate to semicolon or end */
		{
			if (check_is_chain(info, buf, &j))
				break;
			j++;
		}
